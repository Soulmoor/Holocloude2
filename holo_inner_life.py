...
class DailyLifeSimulator:
    ...
    def __init__(self):
        ...
        # NEU: Tägliche Intention
        self.daily_intention: Optional[str] = None
        self.intention_set_date: Optional[str] = None

    def set_daily_intention(self, intention: str = None) -> str:
        """Setze die tägliche Intention/Vorsatz"""
        today = datetime.now().strftime("%Y-%m-%d")

        # Nur wenn neuer Tag oder keine Intention
        if self.intention_set_date != today or not self.daily_intention:
            self.daily_intention = intention or random.choice(self.DAILY_INTENTIONS)
            self.intention_set_date = today

        return self.daily_intention

    def get_daily_intention(self) -> Optional[str]:
        """Hole aktuelle tägliche Intention"""
        today = datetime.now().strftime("%Y-%m-%d")
        if self.intention_set_date == today:
            return self.daily_intention
        return None

    def get_morning_message_with_intention(self) -> str:
        ...
        messages = [
            ...
            f"*gähnt und lächelt* Morgen! Mein Vorsatz für heute: {intention}",
        ]
